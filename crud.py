...
async def start_lnurlcharge(tpos_id: str):
    ...
    if now - int(tpos.withdrawtime) < 10000:
        raise HTTPException(
            status_code=400, detail="Please wait 10 seconds between withdraws"
        )
...
async def update_lnurlcharge(data: LNURLCharge, lnurlcharge_id: str) -> LNURLCharge:
...
async def update_tpos(
    ...
    if timebool:
        ...
        if timebetween < 600000:
            assert (
                tpos
            ), f"Last withdraw was made too recently,  please try again in {(600000 - timebetween) / 1000} secs"
